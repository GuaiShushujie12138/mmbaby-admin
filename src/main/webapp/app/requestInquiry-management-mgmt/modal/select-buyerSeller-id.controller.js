...
(function () {
    ...
    selectBuyerSellerIdCtrl.$inject = ['$scope', '$uibModal','toaster', '$uibModalInstance', 'requestInquiryMgmtService','Identity','opener','id'];

    function selectBuyerSellerIdCtrl($scope, $uibModal,toaster ,$uibModalInstance, requestInquiryMgmtService,Identity,opener,id) {
        var vm = this;

        // 由身份决定选择需求单发起人还是跟进人xuqiudan
        if (Identity === 1) {
            $scope.title = '选择询价单发起人';
        } else {
            $scope.title = '选择询价单跟进人';
        }
    ...
    angular
        ...
        .controller('selectBuyerSellerIdCtrl', selectBuyerSellerIdCtrl);
